...
// int poll(struct pollfd *fds, nfds_t nfds, int timeout);
    ...
    // the timeout expires.
    int numEvents = ::poll(pollFdList_.data(), pollFdList_.size(), timeouMs);
...
void PollPoller::fillActiveChannels(ChannelList *activeChannels, int numEvents) const {
...
void PollPoller::updateChannel(Channel *channel) {
    ...
    if (channel->index()==-1){
        ...
        int idx = static_cast<int>(pollFdList_.size() - 1);
...
void PollPoller::removeChannel(Channel *channel) {
    ...
    if (idx == pollFdList_.size() -1 ){
        ...
        int channelAtEnd = pollFdList_.back().fd;
